...
func Test_DetectFailsNoPnpmLock(t *testing.T) {
	...
	Expect(err).To(MatchError(packit.Fail.WithMessage("no 'pnpm-lock.yaml' file found in the project path %s", filepath.Join(workingDirectory, "project"))))
}
...
func Test_DetectFailsWithoutPackageJsonAccess(t *testing.T) {
	...
	var packageJsonPath = filepath.Join(workingDirectory, "project", "package.json")
	...
	// cleanup
	Expect(os.Chmod(workingDirectory, os.ModePerm)).To(Succeed())
}
