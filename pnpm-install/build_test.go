...
func Test_BuildInstallBuildModulesSuccessfully(t *testing.T) {
	var Expect = NewWithT(t).Expect
	var workingDirectory = t.TempDir()

	mockDependencyService := &fakes.DependencyServiceMock{
		InstallFunc: func(cnbPath string, launch bool) error {
			return nil
		},
	}
...
func Test_BuildInstallLaunchModulesSuccessfully(t *testing.T) {
	var Expect = NewWithT(t).Expect
	var workingDirectory = t.TempDir()
	...
	Expect(layer.Path).To(Equal(filepath.Join(layersDirectory, "launch-modules")))

	// then node_modules should be added to class path
	Expect(layer.LaunchEnv).To(Equal(packit.Environment{
		"PATH.append": filepath.Join(layersDirectory, "launch-modules", "node_modules", ".bin"),
		"PATH.delim":  ":",
	}))
